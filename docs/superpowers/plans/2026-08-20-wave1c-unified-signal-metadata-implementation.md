...
- Stack base is certified Wave 1B2 head `791be6805bb1b83a60c17b3ee0e6ce9010e4ce40`; the approved design/plan branch is `feat/wave1c-unified-signal-metadata-v1`.
- Design baseline `main` is `652f99446ed523c0a602798dde4457bab7983373`; re-check `main` before each new implementation branch.
...
Keep Wave 1C stacked and reviewable rather than one oversized code PR:
...
2. **P1-C2 — Persistence and legacy classification:** Tasks 4–6. Atomic future persistence, explicit producer lineage, deterministic legacy classifier. No report/default consumer cutover until metadata primitives are stable.
3. **P1-C3 — Consumer/runtime cutover and certification:** Tasks 7–10. Canonical readers, STRICT-default report policy, startup completeness gate, fallback guards, Golden/full verification.
...
Create `signal_metadata.py` using frozen Pydantic v2, import `SignalClass`, validate only the two exact class/profile pairs, require 64-char lowercase hex hashes, and require legacy classifications to carry `classification_evidence_hash` while future explicit lineage must not carry one.
...
Open a stacked draft **P1-C1** PR targeting the design/plan branch. Required CI, Sonar, and independent review must pass before P1-C2 starts.
...
Open stacked draft **P1-C2** PR from reviewed P1-C1 head.
...
If all development gates pass, mark P1-C3 `MERGE_READY_PENDING_MERGE_APPROVAL`. If Production-like legacy evidence cannot satisfy completeness, record that as an explicit migration/deployment blocker; do not weaken the gate.
...
- PRs remain stacked/draft/unmerged absent separate approval.
