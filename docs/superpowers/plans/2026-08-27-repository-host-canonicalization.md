...
- `cleanup_legacy_wfh.py --inventory FILE.json --certificate FILE.json --execute` deletes only entries explicitly marked `DELETE_AFTER_CERTIFICATION` and refuses to run unless certification prerequisites are supplied.
