...
Model Excellence phase-0 research follow-up (2026-09-02): commit `12d2cc5` added the normalized compiler; the P0-D analyzer is `scripts/research/analyze_model_crosswalk.py`. With no runtime database/outcomes in this checkout, the checked-in analyzer outputs `research/model_excellence/WFH-ME-V3-20260902/MODEL_LAYER_CROSSWALK.json` and `EVIDENCE_REUSE_MATRIX.json` with status `NOT_RUN_INSUFFICIENT_EVIDENCE`, matched count `0`, and explicit reasons. Current artifact SHA-256 values are `8269fb8d63935cbd7d4e1156dc6d45c9b3b36c7632a340529b8a36565f588c76` (crosswalk) and `a46da68ad132a4304acf9a92c36dfb9dc69044ea701c0eafd05cd87931b67972` (reuse matrix). On a populated export, run `scripts/research/compile_normalized_dataset.py --db <db> --output-dir <dir>` followed by `scripts/research/analyze_model_crosswalk.py --input <dir>/normalized_research.jsonl --output-dir <dir>`. Artifacts are deterministic; hashes and counts must be recomputed for each real export.
