...
CREATE INDEX idx_lbank_signal_ledger_symbol_triggered
ON lbank_signal_ledger (symbol, triggered_at);
CREATE TRIGGER lbank_signal_ledger_no_update
BEFORE UPDATE ON lbank_signal_ledger
BEGIN SELECT RAISE(ABORT, 'lbank_signal_ledger is immutable'); END;
CREATE TRIGGER lbank_signal_ledger_no_delete
...
BEGIN SELECT RAISE(ABORT, 'lbank_signal_outcomes is immutable'); END;
...
CREATE INDEX idx_production_evidence_time ON production_evidence_snapshots(observed_at);
CREATE INDEX idx_production_evidence_symbol ON production_evidence_snapshots(symbol, observed_at);
CREATE TRIGGER production_evidence_no_update
BEFORE UPDATE ON production_evidence_snapshots
BEGIN SELECT RAISE(ABORT, 'production evidence is immutable'); END;
CREATE TRIGGER production_evidence_no_delete
BEFORE DELETE ON production_evidence_snapshots
BEGIN SELECT RAISE(ABORT, 'production evidence is immutable'); END;

CREATE TABLE production_feature_replay_results_v2 (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE INDEX idx_lbank_execution_decision_time
ON lbank_execution_decision_log (last_observed_at);
CREATE INDEX idx_lbank_execution_decision_comparison
ON lbank_execution_decision_log (source, disagreement_kind, bucket_started_at);

CREATE TABLE operational_historical_outcome_datasets (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE TABLE operational_historical_signal_outcomes (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE INDEX idx_operational_historical_symbol
ON operational_historical_signal_outcomes(dataset_id, symbol, signal_timestamp_ms);
CREATE TRIGGER operational_historical_datasets_no_update
...
CREATE TRIGGER operational_historical_datasets_no_delete
BEFORE DELETE ON operational_historical_outcome_datasets
BEGIN SELECT RAISE(ABORT, 'operational historical datasets are immutable'); END;
CREATE TRIGGER operational_historical_outcomes_no_update
...
CREATE TRIGGER operational_historical_outcomes_no_delete
