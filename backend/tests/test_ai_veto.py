...
def test_evaluate_symbol_preserves_the_advisory_provider(monkeypatch):
    ...
    async def opinion(*args, **kwargs):
        return {"advice": "NEUTRAL", "confidence": 42, "reasoning": "live data is mixed", "provider": "gemini"}

    monkeypatch.setattr(engine, "_get_gemini_opinion", opinion)
    vetoed, advisory = asyncio.run(engine.evaluate_symbol(
        "TESTUSDT", {"bids": [[1.0, 1.0]], "asks": [[1.1, 1.0]]}, {"last": 1.0}
    ))
