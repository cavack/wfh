...
class PortfolioEvent(BaseModel):
    ...
    exit_cost: float = Field(default=0.0, ge=0, allow_inf_nan=False)
...
def replay_paper_portfolio(
    ...
    *,
    ...
    if len(dataset_manifest_hash) != 64 or any(
        character not in "0123456789abcdef" for character in dataset_manifest_hash
    ):
    ...
    def total_unrealized() -> float:
        return sum(position.unrealized_pnl() for position in positions.values())

    def mark_equity() -> float:
        return cash_equity + total_unrealized()
    ...
    for event in ordered:
        ...
        if event.event_type is PortfolioEventType.OPEN:
            ...
            if event.rejection_reason:
                status, reason = "SKIPPED", f"ORDER_REJECTED:{event.rejection_reason}"
            ...
            elif plan.get("status") != "READY" or plan.get("execution_mode") != "PAPER_ONLY":
                status, reason = "SKIPPED", "PLAN_NOT_PAPER_READY"
            elif event.position_id in positions:
                status, reason = "SKIPPED", "POSITION_ALREADY_OPEN"
            else:
                position = _position_from_plan(event, plan)
                conservative_equity = cash_equity + min(total_unrealized(), 0.0)
                ...
                if reason is not None:
                    ...
                    cash_equity -= position.entry_cost
            ...
            if position is None:
                status, reason = "IGNORED", "POSITION_NOT_OPEN"
            else:
                amount = float(event.amount or 0.0)
                position.funding += amount
                cash_equity += amount
        elif event.event_type is PortfolioEventType.MARK:
            position = positions.get(event.position_id)
            if position is None:
                status, reason = "IGNORED", "POSITION_NOT_OPEN"
            else:
                ...
                if position.mark_price >= position.liquidation_price:
                    realized = _close_position(
                        ...
                        exit_cost=event.exit_cost,
                    )
                    cash_equity += realized
                    positions.pop(position.position_id)
                    closed_positions.append(
                        _closed_packet(
                            position,
                            event=event,
                            realized_pnl=realized,
                            exit_reason="ISOLATED_LIQUIDATION",
                        )
                    )
                    reason = "ISOLATED_LIQUIDATION"
        elif event.event_type is PortfolioEventType.CLOSE:
            position = positions.get(event.position_id)
            if position is None:
                status, reason = "IGNORED", "POSITION_NOT_OPEN"
            else:
        ...
        event_log.append(
            {
                ...
                "status": status,
                "reason": reason,
                "cash_equity": round(cash_equity, 8),
                "marked_equity": round(mark_equity(), 8),
    ...
    result = {
        ...
        "final_cash_equity": round(cash_equity, 8),
        "final_marked_equity": round(mark_equity(), 8),
        "maximum_drawdown_rate": round(maximum_drawdown, 8),
...
def build_signal_level_research_report(
    ...
    *,
    ...
    ordered = sorted(
        signal_rows,
        key=lambda row: (int(row["signal_triggered_at"]), str(row["signal_id"])),
    )
    report = {
...
def _position_from_plan(event: PortfolioEvent, plan: dict[str, Any]) -> _Position:
    ...
    return _Position(
        ...
        liquidation_price=_number(plan.get("liquidation_price"), "liquidation_price", positive=True),
...
def _closed_packet(
    ...
    *,
    ...
    return {
        ...
        "exit_cost": round(event.exit_cost, 8),
    }
