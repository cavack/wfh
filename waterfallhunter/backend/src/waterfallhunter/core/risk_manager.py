...
def get_leverage(symbol: str) -> int:
    """
    محاسبه و تخصیص اهرم (Leverage) بر اساس نماد معاملاتی.
    """
    # در اینجا می‌توانید منطق داینامیک اهرم را پیاده‌سازی کنید (مثلاً اهرم کمتر برای میم‌کوین‌ها).
    # فعلاً به عنوان یک استاندارد ایمن، اهرم پیش‌فرض را روی 10 تنظیم می‌کنیم.
    return 10
